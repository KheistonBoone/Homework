...
<body>
    <form>
        ...
        <input id="card-search" type="text" name="search-term" />
        <button id="search-btn">Search</button>
    </form>
    <button id="draw-card">Draw One</button>
    <button id="pick-five">Draw Five</button>
    <button id="show-all">Show All</button>
    <div id="deck-of-cards"></div>
</body>
<script>
    const CARDS = [
        {
    ...
    const singleDraw = document.getElementById('draw-card')
    const pickFive = document.getElementById('pick-five')
    ...
    */
    function oneCard() {
        ...
        const newCard = document.createElement('img')
        newCard.setAttribute('src', CARDS[idx].image)
        deck.appendChild(newCard)
    }
    /*
    ...
    */
    ...
    function searchDisplay(e) {
        ...
        for (let j = 0; j <= CARDS.length; j++) {
            if (CARDS[j].value >= searchCard.value) {
                const newCard = document.createElement('img')
                newCard.setAttribute('src', CARDS[j].image)
                deck.appendChild(newCard)
    ...
    showBtn.onclick = showAll;
    singleDraw.onclick = oneCard;
    pickFive.onclick = fiveCard;
    search.onclick = searchDisplay;
